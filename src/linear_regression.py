    ...
    if args.mode == 'validation':
        ...
        for fold in folds:
            ...
            for cfips, df_cfips_group in tqdm(df_train.groupby('cfips'), total=df_train['cfips'].nunique()):
                ...
                X = MinMaxScaler().fit_transform(df_cfips_group[features].bfill())

                X_train = X[df_cfips_group[fold] == 0]
                y_train = df_cfips_group.loc[df_cfips_group[fold] == 0, target].values.reshape(-1, 1)
                X_val = X[df_cfips_group[fold] == 1]

                model = ARIMA(y_train, order=(1, 1, 2))
                model_fit = model.fit()
                y_pred = model_fit.forecast(5)[2:]
                #model = Ridge(1)
                #model.fit(
                #    X=X_train,
                #    y=y_train
                #)
                #y_val = model.predict(X=X_val)
                df_train.loc[val_idx.intersection(np.where(df_train['cfips'] == cfips)[0]), f'{fold}_predictions'] = y_pred
